...
import (
	"bufio"
	"errors"
	"github.com/yuriy-klerk/stungo"
	"github.com/yuriy-klerk/stungo/common"
	stunattrs "github.com/yuriy-klerk/stungo/stun"
	turnattrs "github.com/yuriy-klerk/stungo/turn"
	"net"
	"time"
)
...
func (s *StunClient) readStunPacket() (*stun.Message, error) {
	...
	if header.Length == 0 {
		return stungo.ParseTurn(h, s.Credentials)
	}
	...
	return stungo.ParseTurn(buffer, s.Credentials)
}
...
func (s *StunClient) Bind() (net.Addr, error) {
	...
	packet, err := stungo.NewBindingRequest()
	if err != nil {
		return nil, err
	}
	...
	if response.Header.Type != stungo.BindingResponse {
...
func (s *StunClient) allocateUnauthenticated() error {
	...
	if err := s.send(stungo.NewAllocateRequest(s.Conn.RemoteAddr().Network(), false)); err != nil {
		return err
	}
	s.Credentials = creds
...
func (s *StunClient) Allocate(c *stun.Credentials) (net.Addr, error) {
	...
	if err := s.send(stungo.NewAllocateRequest(s.Conn.RemoteAddr().Network(), true)); err != nil {
		return nil, err
	}
	response, err := s.readStunPacket()
	if err != nil {
	...
	if response.Header.Type != stungo.AllocateResponse {
		msgerr := stunattrs.GetError(response)
		if msgerr.Error() == 442 {
...
func (s *StunClient) RequestPermission(with net.Addr) error {
	...
	if err := s.send(stungo.NewPermissionRequest(addr.HostPart())); err != nil {
		return err
	}
	response, err := s.readStunPacket()
	if err != nil {
	...
	if response.Header.Type != stungo.CreatePermissionResponse {
...
func (s *StunClient) Connect(to net.Addr) (net.Conn, error) {
	if err := s.send(stungo.NewConnectRequest(to)); err != nil {
		return nil, err
	}
	response, err := s.readStunPacket()
	if err != nil {
	...
	if response.Header.Type != stungo.ConnectResponse {
	...
	if err := conn.send(stungo.NewConnectionBindRequest(connectionID)); err != nil {
		conn.Conn.Close()
		return nil, err
	}
	...
	if response.Header.Type != stungo.ConnectionBindResponse {
		conn.Conn.Close()
		return nil, errors.New("Connection failed: " + stunattrs.GetError(response).String())
	}
