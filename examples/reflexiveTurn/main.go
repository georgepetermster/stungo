...
import (
	...
	"github.com/yuriy-klerk/stungo/stun"
	"github.com/yuriy-klerk/stungo/turn"
	"io/ioutil"
	"log"
	"net"
	"net/http"
...
func main() {
	...
	packet, err := stungo.NewAllocateRequest("udp", false)
	if err != nil {
	...
	authResponse, err := stungo.ParseTurn(b[0:n], &packet.Credentials)
	if err != nil {
		log.Fatal("Could not parse authorized AllocateResponse: ", err)
	}

	if authResponse.Header.Type != stungo.AllocateResponse {
	...
	packet, err = stungo.NewPermissionRequest(peerAddr)
	packet.Credentials = credentials
	...
	permissionResponse, err := stungo.ParseTurn(b[0:n], &packet.Credentials)
	if err != nil {
		log.Fatal("Could not parse PermissionResponse: ", err)
	}

	if permissionResponse.Header.Type != stungo.CreatePermissionResponse {
	...
	// Send some data.
	packet, err = stungo.NewSendIndication(myReflexiveAddress.Address, myReflexiveAddress.Port, []byte("Hello World."))
	message, err = packet.Serialize()
	if err != nil {
		log.Fatal("Failed to serialize packet: ", err)
	}
	...
	dataResponse, err := stungo.ParseTurn(b[0:n], &packet.Credentials)
	if err != nil {
		log.Fatal("Could not parse Data Indication: ", err)
	}

	if dataResponse.Header.Type != stungo.DataIndication {
